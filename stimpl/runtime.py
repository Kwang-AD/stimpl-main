...
def evaluate(expression: Expr, state: State) -> Tuple[Optional[Any], Type, State]:
    match expression:
        ...
        case If(condition=condition, true=true, false=false):
            """ Step 1: evaluate the condition in case that it is not just a boolean"""
            condition_value, condition_type, new_state = evaluate(condition, state)
            """Step 2: Check if or not the boolean is the same as condition"""
            ...
            """Step3: Throw error if the feedback is false"""
            """Step4: If the condition evaluates true, evaluate and return the tuple from the true expression"""

            if result:
                return evaluate(true, new_state)
            """Step5: Else, evaluate and reutrn the tuple from the false expression"""
        ...
        case While(condition=condition, body=body):
            ...
            """Step4: If the condition evaluates true, evaluate and go through the body"""
            while result:
