...
def evaluate(expression: Expr, state: State) -> Tuple[Optional[Any], Type, State]:
    match expression:
        ...
        case If(condition=condition, true=true, false=false):
            """Checking value, see if it is just boolean"""
            condition_value, condition_type, new_state = evaluate(condition, state)
            """Checking boolean matching condition"""
            ...
            """If the condition is true, return the tuple as true"""

            if result:
                return evaluate(true, new_state)
            """Else, reutrn the tuple as false"""
        ...
        case While(condition=condition, body=body):
            """Check the value, see if it is just boolean"""
            condition_value, condition_type, new_state = evaluate(condition, state)
            """Check if or not the boolean is the same as condition"""
            match condition_type:
                case Boolean():
                    result = condition_value
                case _:
            ...
            """If true, continue going through the body"""
            while result:
